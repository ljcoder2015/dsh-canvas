...
| F3.8 | 双击全屏预览 | 双击卡片全屏展示产物内容，**弹窗就是整块画布大小**（与画布区同宽同高、不留边距、不切角——旧版是 520px 宽的居中对话框，宽度声明被通用对话框规则顶掉）；头部只有产物名、形态、关闭 ×，**文本节点（Markdown / 文本）另有一枚「编辑 / 预览」切换**（见下条 F3.12）；按形态从查看器注册表取组件：Markdown 渲染、图片/视频、HTML/Deck 沙箱 iframe、CSV/JSON 表格、其余纯文本；产物全文经 `card/read_artifact` 传输（文本 2MB 截断、二进制 16MB 内转 data URL，超限如实提示，不再提示去别处看） |
| F3.12 | 文本就地编辑 | 文本节点（`markdown` / `text` 两个形态，判定收敛在 `isEditableText(kind)`）在预览弹窗里可以**手动改**：切到编辑面就是整块画布大小的等宽编辑区（无边框、tab 缩进 2），⌘/Ctrl+S 或头部「保存」把整篇经既有 `card/write_text` 通道写回磁盘，写完重读一次并让画布刷新（卡片预览与摘要跟着变）。**未保存的修改**头部挂「未保存」标记，此时关闭（× / Esc / 点弹层外）先问「有未保存的修改，仍要关闭吗？」（放弃修改 / 继续编辑）——**不静默丢字**；草稿在预览与编辑之间来回切换时留在原地，回预览看一眼渲染结果不花掉刚打的字。**截断（truncated）的产物不给编辑**：编辑器整篇写回，一次保存会把这一页从未读到的那部分一起覆盖 |
