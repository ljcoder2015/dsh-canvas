...
| F1.5 | 左栏画布区 | 一个包裹而非一排按钮：header 是「画布」标题 + **右侧新建按钮**（点它弹系统文件夹选择器，选定即建画布），下面列出已有画布、点行切换。侧栏收成 rail 时包裹整棵隐藏，只剩一枚画布图标（宿主那一行），**点图标即新建**。选中高亮由宿主全局标准席位 `usePanelInfo`（`activePanelId`）驱动。**画布行右键**就地弹出一张上下文菜单（v1.32）：**打开画布目录**（走宿主的「在应用中打开」路由，只在这台机器真有文件管理器时才出现）与**删除画布**（先确认框，说清「只移出列表、磁盘不动」）——把「这张画布的目录在哪」与「怎么把它从列表里拿掉」放到手边 |
...
| F3.8 | 双击全屏预览 | 双击卡片全屏展示产物内容，**弹窗就是整块画布大小**（与画布区同宽同高、不留边距、不切角——旧版是 520px 宽的居中对话框，宽度声明被通用对话框规则顶掉）；头部只有产物名、形态、关闭 ×，**文本节点（Markdown / 文本）另有一条「预览 / 编辑」单选组**（见下条 F3.12）；按形态从查看器注册表取组件：Markdown 渲染、图片/视频、HTML/Deck 沙箱 iframe、CSV/JSON 表格、其余纯文本；产物全文经 `card/read_artifact` 传输（文本 2MB 截断、二进制 16MB 内转 data URL，超限如实提示，不再提示去别处看） |
