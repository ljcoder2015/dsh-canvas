...
| F3.11 | 卡片控制带 | 选中卡片下方是一条**控制带**：提示词**输入框**（⌘/Ctrl+Enter 发送，空稿禁用发送）与取材 chips、⊕ 引入入口、⤢ 全屏编辑器、模型席位、状态点一行排开。卡片上没有「还没有消息。」一类的状态文字——进度由卡片自己的流光（F3.7）说，产物由卡面预览说；发送入口是条内输入框、⤢ 弹窗与「对话」胶囊三条 |
