...
| F3.8 | 双击全屏预览 | 双击卡片全屏展示产物内容，**弹窗就是整块画布大小**（与画布区同宽同高、不留边距、不切角——旧版是 520px 宽的居中对话框，宽度声明被通用对话框规则顶掉）；按形态从查看器注册表取组件：Markdown 渲染、图片/视频、HTML/Deck 沙箱 iframe、CSV/JSON 表格、其余纯文本；产物全文经 `card/read_artifact` 传输（文本 2MB 截断、二进制 16MB 内转 data URL，超限如实提示） |
