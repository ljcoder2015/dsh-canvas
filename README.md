<p align="center">
  <img src="assets/dsh-canvas-mark.svg" width="104" alt="dsh-canvas 标记：点阵画布上落位的一件产物">
</p>
...
| Agent 可见范围 | 全部内容 | **仅当前卡片 + 取材边指名的上一级产物**（不做穿透引用） |
...
| F1.9 | 画布跟着目录走 | 画布目录**自己记着它是哪张画布**（`.dsh-canvas/board.json` 里的 `id`）：目录改名、搬家、交给同事，重新绑定时认回的是**同一张画布**，座位与取材原样在。判据是「记录里那条旧路径还在不在」——不在＝被搬走了（认回来），还在＝这是拷出来的一份（铸一份自己的新身份，不让两个文件夹共用一个板面）。老画布**零迁移**：目录里还没有这份文件时，身份仍按路径摘要算，所以已存在的画布原地不动 |
| F1.10 | 板面随目录走 | 那份文件同时记着**板面本身**（卡片与座位、取材边、便利贴、风格与名字），座位还带一个只在为真时写的标记「这一格生来就没有产物」（F1.11 那条豁免因此也过得去机器）。标准 JSON、排序输出：可读、可 diff、可手改、模型也能直接读。它是**投影不是真源**——本机读写仍走存储领域，写失败只记日志（目录只读也照常拖拽连线），**读不懂的文件绝不覆盖**（手改到一半或更新格式写的文件不能被旧投影盖掉）；改动后写回一次，**内容没变就不写**。跟着走的是作品的布局与关系，`sessionId`、视图状态这类本机的东西**不跟**（带过去只会假装「有对话」） |
| F1.11 | 清理失效卡片 | 产物已被删除（或改名）的卡片**留在板上**、预览变淡（F3.5），插件从不自动清它（可能只是切了分支）；但过去清起来只有选中面板一条路，攒下十几张就是删整张画布的动机。缩放条上现在有一枚 **清理失效卡片 (N)**，只在真有这种卡时出现，点了先问一句；清的是**卡片**不是文件，拿掉一张会连带忘掉它绑定的对话与取材关系。**「失效」只指产物确实丢了**（探针解析得出这条路径、且明确回答没有这个文件）：**还没写过产物的空座位**（刚落座等着写、位图等着生成）与**探针答不上来的卡**都不算在内、也不会被清，**整个画布目录读不到时整体拒绝**（那时「真没了」与「探针坏了」分辨不出，清空画布请走「移除画布」）；变淡、计数、清理三者共用同一个判据，清完还会说一句实数 |
...
| F3.17 | **改动落到本卡产物** | 同一个卡片的多轮对话**只产出这一份产物**：涉及改动的回合一律**覆盖本卡产物**，不新建卡片、不留 `-v2` / `-copy` 这类第二份让人挑——「再改一下 / 换成…」是**修订**不是新产物，对话本该收敛到一份越改越好的东西。产物是站点 / 应用的入口文件时，同目录那一族文件属于**同一份产物**，照常编辑；越界的只有「另起一份交付物」。**建卡是用户的动作**（拖出节点或明确要求），会话不会因为「想写点什么」顺手建一张；只有用户明确要一份独立交付物时模型才说出来、由用户去建。规则在卡片会话的系统提示词里，**没有机械强制** |
...
| F3.18 | **提示词里的引用标签** | 输入框是 `contenteditable`，`@文件` 记号画成一枚**原子标签**：图标（或缩略图）+ 文件名 + 可选徽标（行号 / 坐标），整体不可分隔——Backspace/Delete 在标签边界删的是**这一枚引用**，标签内部没有光标可落，复制/剪切在剪贴板里还原成 `@路径`，粘贴只取纯文本。**标签只是画上去的**：序列化出去的是它自己那串字符（`@a.ts` / `@"my brief.md"` / `@img.png <point>420 380</point>`），不是从路径重拼的——发出去的提示词逐字不变（拼回去逐字节等于原文，单测与真浏览器判据两头钉住）。类型按扩展名定六类：`code`/`image`/`video`/`audio` 指文件，`mark`/`region` 指图上的点与框（0–999，紧跟文件记号的 `<point>`/`<bbox>` 并进同一枚标签）。**标签写文件名不写序号**：锚点就是路径，模型照它自己读，视觉内容不走第二条通道。`@` 候选菜单：取材来源在前、其余卡片在后（各 8 枚），媒体类带缩略图（分批读，取不到退回图标），**缩略图回来不端走光标**。两处落点（控制带与元素选择的框）是同一个组件；**一枚标签不许把正文那一行撑开** |
...
| F4.3 | 取材持久化 | 作为画布元数据独立存储（不落文件系统）；存储键经**定宽转义**落进 `^[a-zA-Z0-9_-]+$`。**v1.47 起同时写进目录里的板面快照**（F1.10）——本机真源仍是存储领域，那份快照负责让取材跟着目录走；从文件导入的边要过与手绘连线**同一道**校验（自环 / 重复 / 成环 / 端点不在板上，一律丢掉） |
...
| F5.3–F5.5<br>F5.8 | **文件引用（连线的底层）**<br>**取材只取上一级** | 边指向的是**文件**——而这份产物**不一定出自会话**（节点可以手动新建，也可以是对会话产物的二次编辑），所以**拿会话当产物的代理会指错**；插件交出去的因此是**名字**：上游产物按 Harness 自己的 `@file` 语法命名（`@brief.md` / 含空白 `@"my brief.md"` / 目录 `@site/`，与宿主实现逐字节一致），**内容一个字都不复制**——所以不会失效（材料始终是那一份文件）、不占上下文预算、不被截断、没有缓存与失效时机要定义，模型按需 `read`（卡片 id 就是工作区相对路径，正好是会话 cwd，名字直接解析得到）。与**产物摘要**（`read_sources` / `inject_card` / F5.7 `pull`）**并列不替代**：摘要要的是「立刻把材料摆到眼前」，代价是有损 + 有预算 + 每次现算。入口是卡片 ⊕ 菜单的「引用上游产物文件」；返回 `{ files[], skipped[] }`——**语法写不出来的路径（含引号或控制字符）上报而不是悄悄丢掉**。⚠️ 卡片是**文件**不是目录：`site` / `webapp` 是目录形态、卡片却坐落在入口文件上，所以每轮提示里的取材块一律文件形态（`nameWithoutProbe`），只有真探过盘的 `canvas_reference_files` 才加目录尾斜杠。<br>**只取上一级**：三条通道（每轮提示里的取材块 / `read_sources` / `reference_files`）一律**只报直接上游**，不做穿透引用——链要**折**不要**摊**，产物本应把上游材料消化进自己那一份。看全链仍走 F4.7 / `canvas_get_sources`，那是图的问题，不是上下文的问题 |
...
| 左下角「清理失效卡片 (N)」 | 只在真有产物**确实丢了**的卡时出现（空座位、探针答不上的卡都不计入）。点了先问一句，确认后**一次**把 N 张失效卡从板上拿开（连带忘掉它们的对话绑定与取材关系）；**不删任何磁盘文件**。清完报一句实数：确有读不到而被留下的会照实说 |
...
| `sourceDepth` | number | `3` | `getSources` 解析取材链的层数（**只作用于链视图**；取材通道恒为一级，见 F5.8），范围 1–16 |
...
| `canvas_read_sources` | 读取取材来源（**直接**上游产物）的产物摘要——只取上一级，不做穿透引用（F5.8） |
| `canvas_link_source` | 声明本卡片取材于另一张卡片 |
| `canvas_get_sources` | 获取取材链：直接上游、间接上游与下游 |
| `canvas_inject_card` | 把指定卡片产物注入当前会话（`summary` / `full`） |
| `canvas_reference_files` | 把取材来源（**直接**上游产物，上一级）以**文件引用**（`@工作区相对路径`）提交给本卡会话：不复制内容，模型自己按需 `read`；`skipped[]` 说明哪些路径这个语法写不出来 |
...
- **板面投影**（v1.47）：同一条理由的另一面是「画布只活在部署里」——目录改名即换一张画布、换台机器即空板。所以目录里另有一份 `.dsh-canvas/board.json`（`core/canvas/board-file.ts` 定格式与决策，`host/board-file.ts` 接 `ctx.fs` 并从域的表组装）：**投影不是真源**，本机读写仍走存储领域，写失败只记日志而**读不懂的文件绝不覆盖**；改动板面后写回一次，内容没变就不写。
...
│   │   │   ├── cleanup.ts          # 哪张卡可以被批量清掉（三态探针 + 空座位豁免，纯）
