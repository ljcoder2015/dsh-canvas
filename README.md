...
| F1.7 | 指针与键盘 | 指针默认是**选择箭头**；平移握在**空格**里（捕获阶段拦下按压，卡片上方也成立）；WASD / QE 键盘操作；底部 dock：新增（最左，实心圆）+ 快捷键说明弹层 |
...
| F3.8 | 双击全屏预览 | 按形态分派查看器：Markdown 渲染 / 图片视频 / **沙箱 iframe**（`allow-scripts` 无同源）/ CSV·JSON 表格 / 纯文本兜底 |
...
| F3.5 | 状态指示 | 卡片上显示会话状态点 |
| F3.7 | **生成中反馈（流光）** | 回合进入 running，卡片亮起一道斜切 25° 的光带扫过（1.8s 一趟）；名字与预览照常显示。**回合结束流光即停，卡面回显最新产物**。`prefers-reduced-motion` 下光带静止作静态提亮 |
| F3.9 | 提示词回填 | ⤢ 弹窗未编辑时显示用户对该卡片**最近一条自己发出的消息**（逐字保留换行）；插件注入的取材上下文不算用户的话，**不回填** |
| F3.11 | 卡片控制带 | 选中卡片下方是控制带（取材 chips + ⊕、⤢ 提示词弹窗、模型席位、状态点）——**卡片上没有输入框，也没有状态文字**：进度由流光说，产物由卡面预览说 |
