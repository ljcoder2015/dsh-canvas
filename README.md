...
- 质量：`npm run check` 全绿（typecheck + eslint + **321 项**单元测试 + 双端打包）
...
| F3.11 | 卡片控制带 | 选中卡片下方是一条控制带：**提示词输入框**（⌘/Ctrl+Enter 发送，空稿禁用）、取材 chips + ⊕、⤢ 放大输入框、模型席位。**右下角有一颗把手，拖它就是放大这条带子**——放大的只是空间：字号/行高/内边距/三行结构照旧，多出来的地方只归输入框，**锚在卡片中心**（放大时左右两侧对称地长，输入区始终与节点居中对齐），大小按卡记着（本机内存，刷新回默认）。**⤢ 打开的是同一个控制台换了个壳**：还是那三行（材料行 + 输入框 + 底栏），class 序列与行内一字不差、输入框的字号/行高/内边距/圆角逐字相同，草稿也是同一份。**两处按钮各管各的壳**：行内带子右上角那颗是 **⤢ 放大输入框**，弹窗**头部右上角**那颗是 **⤡ 缩小输入框**（它开合的是弹窗本身，所以站在壳的头上，不混进那三行里）。卡片上没有「还没有消息。」一类的状态文字，**也没有状态点**：进度由流光说，产物由卡面预览说 |
...
│       │   ├── composer-size.ts     # 控制带拖动改尺寸的算术（纯）：上下限与 zoom 换算
